...
const static int16_t cc_to_error[16] =
{
	0,					/* No Error		*/
...
const static uint8_t root_hub_config_des[] =
{
	0x09,				/* u8  bLength; */
...
static void dl_del_urb(struct vsfusbh_urb_t *vsfurb)
{
	...
	{
		vsfurb->status = USB_ERR_CONNRESET;
		// vsfsm_post_evt_pending(vsfurb->sm, VSFSM_EVT_URB_REMOVE);
	}
	else
	{
		vsfurb->status = USB_ERR_NOENT;
	}
}
...
static void dl_done_list(struct ohci_t *ohci)
{
	...
	while (td)
	{
		...
		if (ed->state != ED_NEW)
		{
			if (((ed->hwHeadP & 0xfffffff0) == ed->hwTailP) && (ed->state == ED_OPER))
			{
				ep_unlink(ohci, ed);
			}
		}
...
static vsf_err_t rh_submit_urb(struct vsfohci_t *vsfohci,
struct vsfusbh_urb_t *vsfurb)
{
	...
	struct device_request_t *cmd = &vsfurb->setup_packet;
...
static vsf_err_t rh_unlink_urb(struct vsfohci_t *vsfohci,
struct vsfusbh_urb_t *vsfurb)
{
	// TODO: if need int transfer
	return VSFERR_NONE;
...
static vsf_err_t vsfohci_init_thread(struct vsfsm_pt_t *pt, vsfsm_evt_t evt)
{
	...
	err = vsfohci_init_get_resource(usbh, (uint32_t)core_interfaces.ohci.regbase(0));
	if (err)
		return err;
	vsfohci = (struct vsfohci_t *)usbh->hcd_data;

	core_interfaces.ohci.init(OHCI_PORT_INDEX, vsfohci_interrupt, usbh->hcd_data);
...
static vsf_err_t vsfohci_free_device(void *param, struct vsfusbh_device_t *dev)
{
	...
	{
		for (i = 1; i < MAX_EP_NUM_EACH_DEVICE; i++)
		{
			ed = ohci_device->ed[i];
			if (ed->state != ED_NEW)
			{
...
static vsf_err_t vsfohci_submit_urb(void *param, struct vsfusbh_urb_t *vsfurb)
{
	...
	if (usb_pipedevice(pipe) == ohci->rh.devnum)
		return rh_submit_urb(vsfohci, vsfurb);
	...
	if ((!ed) || (ed->state == ED_NEW) || (ed->state & ED_DEL))
	{
		return VSFERR_FAIL;
	}
	...
	vsfurb->status = USB_ERR_URB_PENDING;
	...
	void *delay_free_buf)
{
	...
	if (usb_pipedevice(vsfurb->pipe) == ohci->rh.devnum)
		return rh_unlink_urb(vsfohci, vsfurb);

	if (vsfurb->status == USB_ERR_URB_PENDING)
	{
		if (ohci->disabled)
		{
		...
		else
		{
			...
			{
				vsfurb->status = USB_ERR_INPROGRESS;
				return VSFERR_FAIL;
			}
		}
	}
...
static vsf_err_t vsfohci_relink_urb(void *param, struct vsfusbh_urb_t *vsfurb)
{
	...
	vsfurb->actual_length = 0;
	if ((urb_priv->state != URB_PRIV_DEL) && (vsfurb->status == 0))
	{
		vsfurb->status = USB_ERR_URB_PENDING;
		td_submit_urb(ohci, vsfurb);
		return VSFERR_NONE;
